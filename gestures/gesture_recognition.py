...
from gestures.word_recognition import WordRecognition

class GestureRecognition:
    def __init__(self, use_static_image_mode=False, min_detection_confidence=0.7, min_tracking_confidence=0.7,
                 history_length=16, min_consecutive_letters_recognized=20, min_no_hands_frames_for_word_break=30):
        ...
        # Word Recognition
        self.word = WordRecognition(min_consecutive_letters_recognized, min_no_hands_frames_for_word_break)
    ...
    def recognize(self, image, number=-1, mode=0):
        ...
        # Saving gesture id for ASL detection
        gesture_id = -1
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                hand_sign_id = self.keypoint_classifier(pre_processed_landmark_list, self.min_detection_confidence)
                ...
                hand_sign = ""
                if hand_sign_id == -1:
                    self.word.add_letter(hand_sign_id)
                    hand_sign = "unknown"
                else:
                ...
                self.word.add_letter(hand_sign_id)
                self.word.find_word()
                debug_image = self.draw_guess(debug_image, self.word.curr_word)
            ...
            self.point_history.append([0, 0])
            self.word.add_letter(-2)
            debug_image = self.draw_word(debug_image, self.word.past_word)
    ...
    def draw_point_history(self, image, point_history):
        ## Draws index finger point tracer
        ## Not Currently used
        return image
    ...
    def draw_guess(self, image, guess):
        cv.putText(image, "Guess: " + guess, (220, 400), cv.FONT_HERSHEY_SIMPLEX,
                   1.0, (255, 154, 0), 4, cv.LINE_AA)
        return image
    def draw_word(self, image, word):
        cv.putText(image, "Word: " + word, (220, 400), cv.FONT_HERSHEY_SIMPLEX,
                   1.0, (0, 204, 0), 4, cv.LINE_AA)
        return image
